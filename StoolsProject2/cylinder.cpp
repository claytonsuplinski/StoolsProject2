...
bool Cylinder::Initialize(int slices, float h, float r1, float r2)
{
	...
	for (int i = 0; i < slices; ++i)
	{
		...
		cur_vertexN.position = -cur_vertexN.position;
		dwr_vertexN.position = -dwr_vertexN.position;
		dwn_vertexN.position = -dwn_vertexN.position;
		nxt_vertexN.position = -nxt_vertexN.position;
...
void Cylinder::Draw(const mat4 & projection, mat4 modelview, const ivec2 & size, const float rotY)
{
	...
	//Rotate the object to face the light in the corresponding way.
